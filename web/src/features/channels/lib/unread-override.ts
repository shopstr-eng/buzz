 ...
 * Override Layer). Pure helpers only; no IO.
 *
...
export function splitContexts(wire: Record<string, number>): SplitContexts {
  ...
  const groups = new Map<string, Partial<Record<"s" | "c" | "b", number>> & { bad?: boolean }>();
  ...
  for (const [ctx, g] of groups) {
    ...
    if (has("s") && has("c") && has("b")) {
      overrides[ctx] = { s: g.s!, c: g.c!, b: g.b! };
    } else if (!has("s") && !has("b") && has("c")) {
      overrides[ctx] = { s: 0, c: g.c!, b: 0 }; // tombstone floor
    }
...
export function mergeRegister(a: OverrideRegister, b: OverrideRegister): OverrideRegister {
  return { s: Math.max(a.s, b.s), c: Math.max(a.c, b.c), b: Math.max(a.b, b.b) };
}
...
export function overrideActive(reg: OverrideRegister, frontier: number): boolean {
...
export function planMarkUnread(
  ...
  const register = markUnreadRegister(overrides[ctx], candidateFrontier[ctx] ?? 0);
 ...
 */
export function markReadRegister(reg: OverrideRegister): OverrideRegister | null {
