...
- **Deferred to follow-up tasks**: shared team catalog (kind:30178, relay now gates reads on the `shared` tag), channel topic display/editing.
- **NIP-RS manual-unread override rulings (web)** — group-first validation (complete 3-key group or ov_c-only tombstone; else reject the whole group, keep frontier); clear-wins ties; canonical publication (live→3 keys, dead→tombstone, virgin→omitted); ov_* never budget-evicted; esc: bijection on raw ids starting ov_/esc:. Mark-unread baseline B must include unpublished local markers or the register is dead on arrival and only a tombstone publishes. Merged remote overrides must be mirrored into the local badge store or cross-device sync silently no-ops. The fence+short-page full-state-load shortcut is safe only while web stays single-slot. Desktop implements the layer too; its override helpers must stay semantically identical to web's. Durable rules: overrides are exempt from horizon pruning and budget eviction; ov_* ships only on the primary slot; publish no-op suppression must compare override wire as well as frontier (an override-only mutation changes nothing else); multi-slot clients need the full fence + paged-`until` discharge load, not the single-slot shortcut.
...
- **PersonaShareDialog parity (2026-07-30):** web now has a dedicated persona share dialog — catalog toggle republishes the full 30175 record with the shared tag flipped (never a partial event), memory levels mirror desktop export exactly (core = core entry ONLY, everything = core + all live entries incl. orphans sorted by slug; reachability is a viewer concept, not an export filter), and level "none" must never carry entries (desktop write invariant — normalize, don't trust callers). DM-send/copy-link shipped 2026-07-30: web Blossom client (kind:24242 auth — tags t/x/expiration/server, headers Authorization: Nostr b64 + X-SHA-256, PUT /upload with /media/upload fallback). Snapshot DM message is kind 9 with h tag + NIP-92 imeta tag; content is desktop's imeta markdown file link with a LEADING newline (`\n[label](url)`) — keep byte-parity, don't trim. `.agent.png`/`.team.png` render as file links, never inline images. Fresh-DM resolution waits for the kind:39000 discovery by polling the channels list (relay resolves 41010 idempotently).
